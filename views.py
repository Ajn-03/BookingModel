...
def bookings(request,login_id,rooms_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Bookings.objects.filter(room_name=room,booking_date=b_date,booking_time=b_time,booking_duration=b_dura).first():
                context['message'] = 'Booking already exists!'
                return render(request, 'clovia/booking.html',context)

            else:
                booking = Bookings.objects.create(user=login,room_name=room,booking_date=b_date,booking_time=b_time,booking_duration=b_dura)
...
def edit_booking(request, booking_id,login_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            b_time = form.cleaned_data.get("booking_time")
            b_dura = form.cleaned_data.get("booking_duration")
            if Bookings.objects.filter(id=booking_id,booking_date=b_date,booking_time=b_time,booking_duration=b_dura).first():
                context['form']=form
                context['message'] = 'Booking already exists!'
