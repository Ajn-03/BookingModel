...
class Bookings(models.Model):
    ...
    booking_duration = models.DurationField("Duration")
    participants = models.ManyToManyField(Login, blank=True,related_name='bookings_participated')

    def __str__(self):
        return f"{self.user.username} booked {self.room_name.room} for {self.booking_duration} on {self.booking_date} at {self.booking_time}"
